...
Two Firecrawl indexes sit beside `/scrape`, `/search`, and `/interact`. Neither is reachable through `/search`:
...
The `categories: ["research"]` and `categories: ["developer"]` options on `/search` are website filters. They weight an ordinary web search toward a short list of domains and return page results; they do not query either index. Choose them when a web search is what the feature wants and those sources should be weighed in the same call.
